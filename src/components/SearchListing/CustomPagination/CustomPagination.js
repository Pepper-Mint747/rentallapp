...
class CustomPagination extends Component {
  ...
  renderShowTotal(total, range) {
    return (
      <div className={s.resultsCount}><span>{range[0]}</span><span>&nbsp;–&nbsp;</span><span>{range[1]}</span><span>&nbsp;of&nbsp;</span><span className='paginationRtl'>{total}</span><span>&nbsp;<FormattedMessage {...messages.rentals} /></span></div>
    );
  }
